...
**Arguments**: (`str`, `rules`, `initializer = undefined`, `postTransformers = undefined`)

This is a convenience method which:

1. calls `this.buildParser()` with the supplied `rules`, `initializer`, and `postTransformers`
2. calls `parse()` (on the resulting PegJS parser object) with the supplied `str` string

### buildParser ###

**Arguments**: (`rules`, `initializer = undefined`, `postTransformers = undefined`)

This is a convenience method allowing you to use the same style of API as PegJS which:

1. calls `this.convert()` with the supplied `rules`, `initializer`, and `postTransformers`
...
The optional `initializer` can be a function or curly-bracket-enclosed string to be used as the action.

The optional `postTransformers` object is keyed with rule names (not including labels) containing optional callbacks which will be executed with the first argument as the string generated by processing of the rule with the given rule name and whose return string will be used for transforming the rule; if a function is supplied, it will be executed for each rule with the first argument as the string generated, and the second argument as the rule name.
