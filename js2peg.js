...
* @param {Function|Object} [postTransformers] An object keyed to rule names (not including labels) containing optional callbacks which will be executed with the first argument as the string generated by processing of the rule with the given rule name and whose return string will be used for transforming the rule; if a function is supplied, it will be executed for each rule with the first argument as the string generated, and the second argument as the rule name
* @returns {Object} The PEG parser object
* @example
{
...
*/
js2peg.prototype.parse = function (str, rules, initializer, postTransformers) {
    return this.buildParser(rules, initializer, postTransformers).parse(str);
};

// Allow analogous API to PEG
js2peg.prototype.buildParser = function (rules, initializer, postTransformers) {
    this.convert(rules, initializer, postTransformers);
    // Can call toSource() on parser to get source
    this.parser = PEG.buildParser(this.output, this.parserOptions);
    return this.parser;
};

js2peg.prototype.convert = function (rules, initializer, postTransformers) {
    ...
    this.output += ruleNames.reduce(function (output, ruleNameSeq) {
        ...
        output += _ruleExpressionSeparator + indent + '=' + _postEqualWS;
